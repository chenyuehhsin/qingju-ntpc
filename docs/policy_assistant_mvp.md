...
## 右下角寵物入口更新（2026-09-09）

政策助理改為固定於網頁右下角的「青聚小幫手」芽苗角色，具輕微浮動與眨眼動畫；系統偏好減少動態效果時停用動畫。原本佔用頁面區塊的助理改成可收起面板。

點擊角色展開，按 ×、Esc 或再次點擊角色收起，收起後保留問題與回答。面板獨立捲動，查詢後自動顯示回答；手機版限制在可視範圍內。入口提供 aria-expanded／aria-controls，開啟及關閉處理鍵盤焦點。

本次瀏覽器回歸共 21 項通過（原有 20 項及新增浮動入口互動測試），包含捲動後仍固定右下角、預設收起、開關／Esc／焦點、回答保留與手機尺寸。桌面／手機截圖已檢查。資料查詢、數字、來源與計算方式沿用原有實作。
